...
def indicator_multi_cls(labels, predicted):
...
# %%
def sample_evaluate(init_num, final_num, xtrain, ytrain, xtest, ytest, classifiers, args_train, netG, pos_coeff,
						 device):
	...
	result= indicator_multi_cls(ytest, pred)
...
def DGOT(filepath, testpath, classifiers, oversample_rate, repetitions=20, devices='cuda'):
	...
	model_path = os.path.join(exp_path, f'netG.pth')
	...
	with tqdm(initial=0, total=repetitions) as pbar:
		for k in range(repetitions):
			results = sample_evaluate(init_num, final_num, xtrain, ytrain, xtest,
									  ytest, classifiers, args_train, netG, pos_coeff, device)
			performance.append(results)

			pbar.update(1)

	results = mean_std(performance)
